...
fish_controls <- function(species, selected_vars, var_name) {
  # If species is a character vector (for filtered boxes), use it directly
  # Otherwise, filter from Both2 columns (for non-filtered boxes)
...
sidebar <- dashboardSidebar(width = 150,
                            ...
                            sidebarMenu(
                              ...
                              menuItem("Atlantic Cod", tabName = "AtlanticCod",icon = icon("fish")),
                  ...
                  fluidRow(
                    ...
                    column(width = 8,
                           ...
                           plaice (<35 cm) are preyed on by cod (Gadus morhua), monkfish (Lophius americanus), and spiny dogfish.",
                             style = "font-size: 18px;text-align: justify;"),
                           h4("For more information, click the link below:", style = "padding-top: 40px;"),
                           tags$a(href = "Plaice_LitReview.pdf", "Environmental Effects on American Plaice Stock Dynamics", target = "_blank")
                           )))
AC_info<-tabPanel("Range & Info",
                  fluidRow(
                    column(width = 4,
                           tags$img(src = "Cod_range.png", 
                                    style = "width: 100%; height: 100%; object-fit: cover; margin-top: 30px;")),
                    column(width = 8,
                           h3("Overview"),
                           p("The range of Atlantic cod (Gadus morhua) along the northwestern Atlantic extends from Greenland to Cape Hatteras, North Carolina,
                             with the majority of biomass in United States waters in the western Gulf of Maine (GOM) and Georges Bank (GB) regions. Historically,
                             productivity of the cod fisheries has been high. However, cod in these regions have experienced substantial population declines since
                             the 1990s and according to the most recent GOM and GB Management Track Assessment Reports, both stocks are overfished. These population
                             declines have been attributed to various influencing factors including fishing pressure, an allee effect, or environmental conditions 
                             such as changes in water temperature, or changes in predator or prey composition.",
                             style = "font-size: 18px;text-align: justify;"),
                           h4("For more information, click the link below:", style = "padding-top: 40px;"),
                           tags$a(href = "Cod_LitReview.pdf", "Environmental Effects on Atlantic Cod Stock Dynamics", target = "_blank")
                    )))
...
for (col_name in colnames(Both2)) {
  ...
  y_name2 <- gsub("\\b(Atlantic|River|American|Landings|Striped Bass|lobster|Biomass|Plaice|Cod)\\b", "", y_name)
...
plot_function<-function(plottingstyle,num_variables=num_variables, dataDf=dataDf, all_vars=all_vars, name_mapping=name_mapping, name_mapping2=name_mapping2, show_trendline, show_lta){
  ...
  # Calculate Long Term Averages if show_lta is TRUE
  lta_lines <- lapply(all_vars, function(var) {
    if (show_lta) {
      mean(dataDf()[[var]], na.rm = TRUE)
    } else {
      NA
    }
  })
  ...
  if (plottingstyle == "Layered") {
    ...
    if (num_variables > 4){ #plot 5 variables together
      ...
      # Add LTA lines as constant vectors
      lta_data <- data.frame(
        Year = dataDf()$Year,
        LTA1 = rep(lta_lines[[1]], nrow(dataDf())),
        LTA2 = rep(lta_lines[[2]], nrow(dataDf())),
        LTA3 = rep(lta_lines[[3]], nrow(dataDf())),
        LTA4 = rep(lta_lines[[4]], nrow(dataDf())),
        LTA5 = rep(lta_lines[[5]], nrow(dataDf()))
      )
      plot_ly(dataDf(), x = ~Year, y =~get(all_vars[1]), 
        ...
        # Add LTA lines if show_lta is TRUE
        add_trace(data = lta_data, x = ~Year, y = ~LTA1, type = 'scatter', mode = 'lines',
                  name = paste(name_mapping[all_vars[1]], "LTA"), line = list(dash = 'solid', color = "#00608A")) %>%
        add_trace(data = lta_data, x = ~Year, y = ~LTA2, type = 'scatter', mode = 'lines',
                  name = paste(name_mapping[all_vars[2]], "LTA"), line = list(dash = 'solid', color = "#ABB400")) %>%
        add_trace(data = lta_data, x = ~Year, y = ~LTA3, type = 'scatter', mode = 'lines',
                  name = paste(name_mapping[all_vars[3]], "LTA"), line = list(dash = 'solid', color = "#EA4F12")) %>%
        add_trace(data = lta_data, x = ~Year, y = ~LTA4, type = 'scatter', mode = 'lines',
                  name = paste(name_mapping[all_vars[4]], "LTA"), line = list(dash = 'solid', color = "#683562")) %>%
        add_trace(data = lta_data, x = ~Year, y = ~LTA5, type = 'scatter', mode = 'lines',
                  name = paste(name_mapping[all_vars[5]], "LTA"), line = list(dash = 'solid', color = "#FF7272")) %>%
    ...
    plot_list <- lapply(1:num_variables, function(i) {
      ...
      if (is.null(all_vars[i]) || all_vars[i] == "") {
      ...
      lta_values <- rep(NA, nrow(dataDf()))
      # Conditionally calculate trendline
      if (show_trendline) {
        if (!all(is.na(dataDf()[[all_vars[i]]]))) {
      ...
      # Conditionally calculate LTA
      if (show_lta) {
        if (!all(is.na(dataDf()[[all_vars[i]]]))) {
          lta_value <- mean(dataDf()[[all_vars[i]]], na.rm = TRUE)
          lta_values <- rep(lta_value, nrow(dataDf()))
        }
      }
      # Assign color from a consistent palette
      colors <- c("#00608A", "#ABB400", "#EA4F12", "#683562", "#FF7272", "black")
      color <- colors[(i - 1) %% length(colors) + 1]
      # Create plot
      plot <- plot_ly(dataDf(), x = ~Year, y = as.formula(paste0("~", all_vars[i])), 
                      type = 'scatter', mode = 'lines', name = y_name, 
                      line = list(color = color), showlegend = FALSE)
      ...
      # Add LTA line if enabled
      if (show_lta) {
        plot <- plot %>%
          add_trace(x = ~Year, y = ~lta_values,
                    type = 'scatter', mode = 'lines', name = paste(y_name, "LTA"),
                    line = list(dash = 'solid', width = 2, color = "#bdbdbd"),
                    showlegend = FALSE)
      }
      # Add layout and annotations
      plot <- plot %>%
        layout(
          yaxis = list(title = y_name2, side = 'left', showgrid = FALSE, zeroline = FALSE),
    ...
    if (num_variables == 0) {
      stop("No variables selected to create plots.")
    }
