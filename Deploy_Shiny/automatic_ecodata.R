...
other_df <- other_df %>%
  ...
  ungroup()
other_df$Var <- paste0("Gulf_Stream_Index")
other_df <-distinct(other_df)

ecodata_df<-rbind(ecodata_df,other_df)
####NAO dataset#######
other_df<-as.data.frame(ecodata::nao[c(1:4)])
other_df$Var <- paste0("North_Atlantic_Oscillation")
...
names(amo_data)[1] <- "Atlantic_Multidecadal_Oscillation"
amo_data$Atlantic_Multidecadal_Oscillation<-as.character(amo_data$Atlantic_Multidecadal_Oscillation)
amo_data[c('Year', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')] <- str_split_fixed(amo_data$Atlantic_Multidecadal_Oscillation,'   ',13)# Split name column into mulitple columns by month
...
amo_data$Atlantic_Multidecadal_Oscillation_Annual_Mean<-rowMeans(amo_data[ ,2:13]) ###find annual average
